  ...
  <div class="container-fluid">
    ...
    <table class="table table-bordered table-striped" id="machines_table">
      ...
      <tbody>
        ...
        <tr>
          <td>{{$m->machine_id}}</td>
          <td>{{$m->type->machine_type_name}}</td>
          <td>{{$m->machine_name == '' ? '-' : $m->machine_name}}</td>
          <td>{{$m->model_number == '' ? '-' : $m->model_number}}</td>
          <td>{{$m->machine_purchase_date}}</td>
          <td>{{$m->machine_availability == '1' ? 'Available' : 'Not available'}}</td>
          <td>{{$m->additional_details == '' ? '-' : $m->additional_details}}</td>
          <td>
            <div class="btn-group" role="group">
              <a href="{{route('admin.machines.edit', $m->machine_id)}}" class="btn btn-info"> Edit </a>
              <a href="{{route('admin/machines/{machine_id}/allocationEdit',$m->machine_id)}}" class="btn btn-warning">
                Allocate </a>
            </div>
          </td>
          @endforeach
