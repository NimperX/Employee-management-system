      ...
      <div class="container-fluid">
        <form class="form-horizontal" action="{{route('admin.warranties.update', $warranty->warranty_id)}}" method="POST">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10">
              <select class="form-control" name="project_id" required>
                <option value="" disabled selected value="{{$warranty->project->project_id}}"> Choose your option </option>
                @foreach($projects as $p)
                <option value="{{$p->project_id}}" {{$warranty->project->project_id == $p->project_id ? 'selected' : ''}}> {{$p->project_id}} - {{$p->project_name}} , {{$p->project_location}} ({{$p->customer->company_name}}) </option>
                @endforeach
              </select>
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10">
              <textarea class="form-control" name="description" rows="7">{{$warranty->description}}</textarea>
            </div>
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10">
              <input type="text" placeholder="Start date" onfocus="(this.type='date')" onblur="(this.type='text')"
                name="warranty_start_date" value="{{$warranty->warranty_start_date}}">
              <input type="text" placeholder="End date" onfocus="(this.type='date')" onblur="(this.type='text')"
                name="warranty_end_date" value="{{$warranty->warranty_end_date}}">
            </div>
          </div>
